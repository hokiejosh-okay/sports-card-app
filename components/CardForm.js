...
function assemble(form, uid, photos) {
...
CV.saveCardFromForm = async function (mode, existingCard, form, photos) {
  const uid = CV.auth.currentUser.uid;
  const cardId = mode === "edit" ? existingCard.id : CV.newCardId();
...
CV.CardForm = function CardForm(props) {
  ...
  async function onSave() {
    ...
    setSaveError("");
    try {
      const saved = await CV.saveCardFromForm(mode, existing, form, photos);
      props.onSaved && props.onSaved(saved);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="cardform">
      ...
      <div className="form-section">
        <label className="field-label">Photos <span className="req">both required</span></label>
      ...
      <div className="form-grid">
        <Field label="Player" error={errors.player} full>
        ...
        <Field label="Serial #" hint="e.g. 12/99">
      ...
      <div className="form-section">
        <div className="graded-head">
          <label className="field-label" style={{ margin: 0 }}>Graded</label>
          <Toggle label={form.graded ? "Graded" : "Raw"} on={form.graded} onClick={() => set("graded", !form.graded)} />
        </div>
        {form.graded ? (
          <div className="form-grid">
            <Field label="Company" error={errors.gradingCompany}>
            ...
            <Field label="Grade" error={errors.gradingGrade}>
